...
- **Next / finish line:** Release the bounded frontend repair after checks; in the original iPhone browser (without clearing data), save one real mission-linked action, start it, save a starting-point note and pause, reload, then resume the same action. Record the same action ID, note, status and absence of duplicates/errors. A fresh anonymous session or code tests cannot satisfy this finish line.
